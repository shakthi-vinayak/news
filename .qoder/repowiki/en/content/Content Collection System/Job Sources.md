...
A --> G["greenhouse.py"]
A --> H["lever.py"]
A --> I["dedupe.py<br/>make_job_id()"]
A --> J["db.py<br/>upsert_job()"]
A --> K["export_json.py<br/>export_all()"]
L["config.yaml<br/>jobs.* configs"] --> A
```
...
- id, title, company, url, source, location, posted_at, category, relevance_score, salary_range
...
## Dependency Analysis
- Configuration-driven: Each collector reads its own section from config.yaml (e.g., jobs.remoteok, jobs.remotive, jobs.weworkremotely, jobs.arbeitnow, jobs.hn_whoishiring, jobs.greenhouse, jobs.lever).
...
## Conclusion
Each job source follows a consistent pattern: fetch data from a public endpoint or feed, normalize fields, apply keyword filters, and produce a standardized job item. The orchestrator centralizes configuration, deduplication, persistence, and export. Robustness is achieved through defensive parsing, fallbacks for missing data, and per-source error logging. For future enhancements, consider adding retry/backoff, configurable delays, and schema validation for incoming payloads.