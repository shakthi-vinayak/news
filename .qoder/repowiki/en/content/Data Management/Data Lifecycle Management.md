...
## Update Summary
**Changes Made**
- Updated retention filtering section to reflect separate retention policies for news (7 days) and jobs (60 days)
- Enhanced timestamp handling documentation with first_seen_at for expiration calculations
- Added documentation for massive pre-population of news (1,284 entries) and jobs (460 entries) datasets
- Updated database query examples to show separate retention windows for different content types
- Enhanced Windows compatibility documentation with improved Unicode character support
...
**Updated** Enhanced with separate retention policies for different content types and improved timezone handling for cross-platform compatibility.
...
- Configuration: retention windows for different content types, LLM parameters, keyword filters, and source-specific settings.
...
**Updated** Enhanced with separate retention policies and massive pre-population capabilities.
...
The lifecycle spans from collection to export and publication. The retention policy is enforced during export to limit the dataset to recent items using separate retention windows for different content types.
...
Main->>Export : export_all(retention_days, jobs_retention_days)
Export->>DB : get_news(days=7) and get_jobs(days=60) with first_seen_at filtering
...
**Updated** Separate retention policies now apply different time windows: 7 days for news and 60 days for jobs, with enhanced timestamp handling using first_seen_at.
...
- Queries: retrieval functions apply separate date thresholds to limit results to configured retention windows using first_seen_at timestamps.

**Updated** Enhanced retention filtering now uses separate time windows: 7 days for news and 60 days for jobs, with first_seen_at timestamp for consistent cross-platform operation.
...
### Export Pipeline
- Reads from the database using retention-aware getters with separate time windows for different content types.
...
```mermaid
flowchart TD
Start(["Export Entry"]) --> GetNews["get_news(days=7)<br/>Uses first_seen_at with 7-day retention"]
...
Start --> GetJobs["get_jobs(days=60)<br/>Uses first_seen_at with 60-day retention"]
...
**Updated** Separate retention policies now apply different time windows: 7 days for news and 60 days for jobs, with enhanced timestamp handling for consistent filtering.
...
- Loads configuration including separate retention_days and jobs_retention_days.
- Applies keyword pre-filter and fuzzy deduplication before scoring.
- Persists items to the database and exports JSON with separate retention windows using first_seen_at timestamps.
- Records run metadata and optionally publishes to Git.

**Updated** Enhanced with separate retention policies for different content types and improved timezone handling.
...
Main->>Cfg : Load retention_days (7) and jobs_retention_days (60)
Main->>Dedup : dedupe_batch() + keyword filter
Main->>LLM : score_news_batch()/score_jobs_batch()
Main->>DB : upsert_* within transaction
Main->>Export : export_all(retention_days=7, jobs_retention_days=60)
Export->>DB : get_news(days=7) and get_jobs(days=60) with first_seen_at filtering
Export-->>Main : Counts and logs
```

**Updated** Separate retention policies now apply 7-day window for news and 60-day window for jobs, with enhanced timestamp handling for consistent cross-platform operation.
...
- **Jobs Dataset**: 460 pre-populated job listings from major technology companies  
...
- [docs/data/jobs.json:1-200](file://docs/data/jobs.json#L1-L200)
- [worker/storage/db.py:22-67](file://worker/storage/db.py#L22-L67)
...
- The export layer depends on the database getters to enforce retention with separate time windows.
...
- Retention windows: Separate retention_days (7) and jobs_retention_days (60) settings limit query sizes and export payloads, reducing memory and IO overhead.
...
**Updated** Added separate retention window considerations and Windows compatibility improvements.
...
- **Retention Issues**: The system now uses separate retention windows (7 days for news, 60 days for jobs) with first_seen_at timestamp handling, resolving cross-platform date comparison issues.
- **Unicode Problems**: Enhanced UTF-8 encoding support resolves character display issues on Windows and other platforms.

**Updated** Added troubleshooting guidance for separate retention policies and Unicode-related issues.
...
## Conclusion
The system enforces separate retention windows during export using first_seen_at timestamps, with 7 days for news and 60 days for jobs. The database schema supports efficient queries, and the export process produces validated JSON artifacts with enhanced Unicode support. Automation via GitHub Actions or Docker Compose enables reliable scheduling, while massive pre-population capabilities facilitate testing and development. The improved timezone handling ensures consistent operation across different platforms and environments.

**Updated** Enhanced with separate retention policies and improved cross-platform compatibility.

## Appendices

### Data Retention and Cleanup Procedures
- Retention policy: Controlled by separate retention_days (7) and jobs_retention_days (60) in configuration. Export functions filter items using first_seen_at timestamps with different time windows for each content type.
- Automatic cleanup: No explicit pruning script exists; retention is enforced at read/export time with separate windows.
- **Enhanced Timestamp Handling**: Queries now use first_seen_at with proper timezone offset calculations, ensuring consistent filtering across different platforms and timezones.
- Practical examples:
  - Adjust retention_days to control news dataset size and export time.
  - Adjust jobs_retention_days to manage job listing archive size.
...
**Updated** Enhanced with separate retention windows and timezone-aware filtering improvements.
...
- Use separate retention_days (7) and jobs_retention_days (60) to cap dataset sizes.
...
- Changing retention windows:
  - Adjust retention_days (7) and jobs_retention_days (60); re-run the pipeline to regenerate JSON with the new windows.
...
- **Jobs Dataset**: 460 entries from major tech companies
- **Usage Scenarios**:
